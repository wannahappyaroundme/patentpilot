...
function asGrades(v: string | undefined): string[] {
  if (!v) return [];
  return v
    .split(",")
    .map((s) => s.trim().toUpperCase())
    .filter((g) => (VALID_GRADES as readonly string[]).includes(g));
}
  ...
  return (
    <div className="py-8">
      <PrintHeader
        ...
        subtitle={`총 ${formatNumber(result.total)}건 · 필터: urgency=${sp("urgency") ?? "ALL"} org=${sp("org") ?? "ALL"} ipc=${sp("ipc") ?? "ALL"} q=${sp("q") ?? "-"} sort=${sort}${grades.length ? ` grade=${grades.join(",")}` : ""}`}
      />
